...
if [ -n "${CI_ENABLE_COVERAGE:-}" ]; then
  ...
  export SONAR_TOKEN=$($VAULT_HOME/vault kv get -field token secret/inspec/train-winrm/sonar)
...
#TODO: If coverage is enabled, then we need to pick up the coverage/coverage.json file
if [ -n "${CI_ENABLE_COVERAGE:-}" ]; then
